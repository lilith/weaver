...
{#if data.user}
	<section class="space-y-6 py-10">
		<h1 class="font-display text-5xl text-mist-100">
			Welcome back, <span class="font-hand text-4xl text-candle-300">{data.user.display_name}</span>.
		</h1>
		<p class="text-mist-400">Your worlds are waiting.</p>
		<a href="/worlds" class="storybook-button no-underline">Go to your worlds</a>
	</section>
{:else}
	<section class="space-y-8 py-10 sm:py-16">
		<header class="space-y-3">
			<h1 class="font-display text-5xl tracking-tight text-mist-100 sm:text-6xl">
				Weaver
			</h1>
			<p class="font-hand text-2xl text-candle-300">a small loom for stories, shared</p>
		...
		{#if form?.ok}
			<div class="story-card px-5 py-4">
				<p class="text-mist-400">
					<span class="font-hand text-xl text-candle-300">sent.</span> Check <strong>{form.email}</strong>
		...
		{:else}
			...
			>
				<label class="block space-y-1.5">
					<span class="text-sm text-mist-400">your email</span>
					<input
						id="email"
						name="email"
						type="email"
