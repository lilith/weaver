...
- **Quiet Vale backup to a separate repo** — user-flagged 2026-04-20. Design spec pending (probably folds into `HOUSEHOLD_AND_SHARING.md`).

## Revisions (2026-04-20 second pass — design-direction session)

Design session covering progression, multiplayer under async play, art, sprawl-resistance. All decisions land as `designed` status in `FEATURE_REGISTRY.md`; none ship before playtest.

| Decision | Rationale |
|---|---|
| **Async-first campaign model** — clock monotonic, advances when any character acts; late-arriving players catch up via per-event "I was with them / I skipped it / tell me about it" panels. Retroactive agency, no race. See `ASYNC_SYNC_PLAY.md`. | Family play is asynchronous by necessity. Real-time-MMO doesn't fit; strict-turn-based blocks. Campaign model borrows from PBeM / book-club pacing and makes stolen-moment play *and* family-LAN night both work. |
| **Eras + per-character personal_era** — per-entity × per-era versioning, world.active_era canonical, characters.personal_era tracks acknowledged beats. Gating enforces "key stuff must be experienced by all eligible." See `25_ERAS_AND_PROGRESSION.md`. | Browsing end-game state isn't fun; file-level peeking spoils story. Era-filtering the bible at prompt-time prevents AI spoilers; entity-visibility gating blocks premature wandering; personal-era gating ensures beats land without forcing sync. |
| **One file per era for authoring** (`foo.era-2.md` sibling files; inheritance falls back to prior eras). | Each era's authoring diffs cleanly; reviewable standalone. Importer combines into `era_version_map`. |
| **Background arc_pressure** — authored pressure variables tick up per turn, seep into prose, surface in catch-up panels, trigger arc beats at thresholds. | Arc beats feel earned, not imposed. Pressure is Chekhov's gun seeding. |
| **Era transitions as AI-synthesized stage-shifts** — ~500-word skeleton per era + Opus producing family-adapted per-entity era-versions. | Author writes 4 skeletons for a 5-era campaign in a weekend. Each family's arc is unique while hitting authored beats. ~$1-2 per transition. |
| **Forced-choice arc beats** — authored events include "family picks who" mechanisms for skulljack-style offers and paralysis-style costs. | Railroad-free while preserving arc's authority. Family keeps hand on tiller even during mandatory shifts. |
| **Text-only art default + eye-icon affordance** — no auto-gen; no hero-image-at-top; no "forming…" spinner. Click eye to conjure. See `ART_CURATION.md`. | AI art lacks continuity; photoreal amplifies uncanny valley. Opt-in means every visible image was summoned deliberately. Art cost drops ~$5-10/wk → ~$0.50-1.50/wk. |
| **Wardrobe of art modes, communal variants** — `entity_art_renderings` table; multiple variants per mode per entity; visitors cycle, regen, vote; changes world-visible. Per-character preference picks default-mode-on-visit only. | One style isn't enough; forcing one is constraining. Wardrobe lets families try treatments per location. Communal voting converges taste over weeks; reference board feeds FLUX as refs so art compounds toward family aesthetic. |
| **Stylization over photorealism** — wizard quick-picks omit photoreal; nudge copy encourages visible style. | Photoreal amplifies drift; stylized absorbs it. Encoding the lesson up front. |
| **Predictive text prefetch** — click-into-nowhere only; up to 3 options; budget-gated; `draft: true, visited_at: null` pre-committed-pending invariant. | 8s expansion waits are the worst part of current play. Prefetch during reading makes hot case near-instant. ~$0.30/wk cost. Adults on, minors off default. |
| **Expansion streaming** — Opus stream + skeleton-first render + in-world placeholder line pool. | Turns 6s of blank into 6s of reading. Streaming tokens render prose as it writes. |
| **UX-01 time-gated option hiding — (b) gray-out-with-hint v1, (d) authored `hidden_until` / `teaser_when` later.** | Silent disappearance is worse than a vague hint. Fine-grained control ships after simple version is playtested. |
| **UX-03 wait affordance — try global "wait a moment" button.** | Playtest-driven. Alternative (every-location implicit) is heavier. |
| **Retrofit both worlds to no-art default immediately** — Quiet Vale + The Office flip to wardrobe; legacy art becomes `hero_full` variants. | Policy is world-level, not opt-in. No data loss; art stays accessible via wardrobe. |
| **Feature registry + flag runtime** — every feature carries status / flag / seam / rollback. `feature_flags` table gates with scope precedence (character → user → world → global). See `FEATURE_REGISTRY.md`. | Sprawl resistance: pulling a feature becomes one flag flip + code archive, not archaeology. Status-header on every spec keeps feature state legible across context-compaction. |
| **PLAYTEST_LOG.md** — dated per-feature observations drive flag transitions. | Flag status changes are evidence-driven, not vibes-driven. |
| **Directory pattern `convex/features/<name>/` + `packages/engine/features/<name>/`** — each feature self-contained; pulling = rename to `features/pulled/<name>/`. | Physical modularity enforces logical modularity. Cross-feature imports through `index.ts` only; scattered coupling is a review smell. |
